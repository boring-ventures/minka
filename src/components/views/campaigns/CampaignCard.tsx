...
export function CampaignCard({ title, image, category, location, progress, verified }: CampaignCardProps) {
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-shadow border border-gray-100">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center text-sm mb-4 gap-2">
          <span className="bg-[#e8f0e9] text-[#2c6e49] px-3 py-1 rounded-full">{category}</span>
          <span className="bg-[#e8f0e9] text-[#2c6e49] px-3 py-1 rounded-full">{location}</span>
        </div>
        <div className="mb-4">
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-[#2c6e49] h-2 rounded-full" style={{ width: `${progress}%` }} />
          </div>
