...
const faqItems = [
  {
    ...
    answer: "La verificación aumenta la confianza de los donadores y asegura la transparencia de tu causa.",
  },
  {
  ...
  },
  {
    ...
    answer: "Sí, puedes donar el monto que desees. Cada aporte cuenta y hace la diferencia en las causas que apoyas.",
  },
  {
  ...
  return (
    <Accordion type="single" collapsible className="mb-16">
      {faqItems.map((item, index) => (
        <AccordionItem key={index} value={`item-${index}`} className="border-b border-[#478C5C]/20">
          <AccordionTrigger className="text-xl font-medium text-[#2c6e49] hover:text-[#2c6e49]/90 py-6">
            {item.question}
          </AccordionTrigger>
          <AccordionContent className="text-lg text-gray-600 py-4">{item.answer}</AccordionContent>
        </AccordionItem>
      ))}
    </Accordion>
  )
